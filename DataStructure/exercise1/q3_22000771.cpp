...
int n_high_score(node *p, double t) {
  ...
  if (p->left != NULL && p->left->score > t) {
    result += n_high_score(p->left, t);
  }
  if (p->right != NULL && p->right->score > t) {
    result += n_high_score(p->right, t);
  }
  // result += n_high_score(p->left, t) + n_high_score(p->right, t);
  return result;
}
...
double get_maxscore(node *p) {
  double result = p->score;
  if (p == NULL) {
    return 0;
  }
  if (p->left != NULL) {
    if (p->left->score > result) {
      result = p->left->score;
      result = get_maxscore(p->left);
    }
  }
  if (p->right != NULL) {
    if (p->right->score > result) {
      result = p->right->score;
      result = get_maxscore(p->right);
    }
  }
  return result;
}

double my_tree::max_score() { return get_maxscore(root); }
...
double get_minscore(node *p) {
  ...
  if (p->left != NULL) {
    if (p->left->score < result) {
      result = p->left->score;
      result = get_minscore(p->left);
    }
  }
  if (p->right != NULL) {
    if (p->right->score < result) {
      result = p->right->score;
      result = get_minscore(p->right);
    }
  }
  return result;
}
double my_tree::min_score() { return get_minscore(root); }

double my_tree::d_min_max() { return max_score() - min_score(); }
