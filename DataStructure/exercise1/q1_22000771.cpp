...
int count_leaf_nodes(node *p) {
  ...
  leafNodes += count_leaf_nodes(p->left) + count_leaf_nodes(p->right);
  return leafNodes;
}

int my_tree::n_leaf_nodes() { return count_leaf_nodes(root); }
...
int count_d1_nodes(node *p) {
  ...
  degree1Nodes += count_d1_nodes(p->right) + count_d1_nodes(p->left);
  return degree1Nodes;
}

int my_tree::n_d1_nodes() { return count_d1_nodes(root); }
...
int count_d2_nodes(node *p) {
  ...
  d2Nodes += count_d2_nodes(p->right) + count_d2_nodes(p->left);
  return d2Nodes;
}

int my_tree::n_d2_nodes() { return count_d2_nodes(root); }
